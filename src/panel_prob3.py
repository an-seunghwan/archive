...
#%%
p1 = gagu2017['가구원수'][gagu2017['가구원수'] == 1].sum() / gagu2017['가구원수'].sum()
p3 = gagu2019['가구원수'][gagu2019['가구원수'] == 1].sum() / gagu2019['가구원수'].sum()
#%%
'''가구 데이터와 인구 데이터 병합'''
df17 = pd.merge(gagu2017[['가구번호', '가구원수']], ingu2017[['가구번호', '가구원번호', '성별코드', '만나이']], how='inner', on='가구번호')
df19 = pd.merge(gagu2019[['가구번호', '가구원수']], [['가구번호', '가구원번호', '성별코드', '만나이']], how='inner', on='가구번호')
#%%
...
result
#%%
# ingu2017[ingu2017['가구번호'] == '15-0000003']
# ingu2019[ingu2019['가구번호'] == '15-0000003']

# (ingu2019['가구원번호'] == '15-0199259').sum()
#%%
alpha = result[1][1, 1] / np.sum(result[1][1, :])
beta = (alpha * p1 - p3 + 1 - p1) / (1 - p1)
#%%
df.to_csv('./panel_seoul.csv', encoding='cp949')
#%%
...
result
#%%
df.to_csv('./panel_not_seoul.csv', encoding='cp949')
#%%